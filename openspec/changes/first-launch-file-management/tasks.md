...
- [ ] 1.1 Add `tauri-plugin-dialog` and `tauri-plugin-fs` crates to `src-tauri/Cargo.toml`
- [ ] 1.2 Register `tauri_plugin_dialog` and `tauri_plugin_fs` in `src-tauri/src/lib.rs`
- [ ] 1.3 Declare required permissions in the Tauri capability file (`dialog:allow-open`, `dialog:allow-save`, `fs:allow-copy-file`, `fs:allow-read-dir`, `fs:allow-remove-file`)
