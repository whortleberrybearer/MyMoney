...
- [ ] 1.1 Add `transactionFitid` table to `src/lib/db/schema.ts` with columns `id`, `transactionId`, `accountId`, `fitid` and a unique index on `(accountId, fitid)`
- [ ] 1.2 Run `drizzle-kit generate` to produce a new migration SQL file
