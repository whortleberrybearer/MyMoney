...
- [ ] 1.1 Update `src/lib/db/schema.ts`: add `payee TEXT`, `reference TEXT`, `categoryId INTEGER` (FK to `category.id`), `runningBalance REAL NOT NULL DEFAULT 0` to the `transaction` table; remove `description TEXT`
- [ ] 1.2 Run `npx drizzle-kit generate` to produce the Drizzle migration file and commit it alongside the schema change
